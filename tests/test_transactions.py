...
def auth_setup():
    ...
    database.users[1] = {"id": 1, "name": "Sender", "role": "USER", "balance": 1000.0}
    database.users[2] = {"id": 2, "name": "Receiver", "role": "USER", "balance": 0.0}
...
def test_add_transaction_success(mock_handler_factory, auth_setup, monkeypatch):
    ...
    monkeypatch.setattr(Transaction, "create", MagicMock(return_value=(1, None)))
...
def test_get_transactions_success(mock_handler_factory, auth_setup, monkeypatch):
    ...
    monkeypatch.setattr(Transaction, "get_all", MagicMock(return_value=database.transactions))
...
def test_get_my_transactions_success(mock_handler_factory, auth_setup, monkeypatch):
    ...
    monkeypatch.setattr(Transaction, "get_by_user", MagicMock(return_value=database.transactions))
