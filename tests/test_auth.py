...
def test_login_success(mock_handler_factory, monkeypatch):
    ...
    monkeypatch.setattr(User, "get_by_email", lambda email: user if email == "test@example.com" else None)
...
def test_logout_success(mock_handler_factory):
    ...
    handler = mock_handler_factory(headers={"Authorization": f"Bearer {token}"})
